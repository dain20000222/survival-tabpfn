...
# CSV path
csv_path = "xgboost_evaluation.csv"

def construct_xgboost_binary_trainset(x_train, y_train, times):
    """
    Construct XGBoost training dataset using binary classification approach.
    ...
    Returns:
    - X_xgboost: DataFrame of features for XGBoost
    - y_xgboost: Series of binary labels for XGBoost
    """
    ...
    X_xgboost = pd.DataFrame(dataset_rows, columns=feature_cols)
    y_xgboost = pd.Series(binary_labels)
...
def construct_xgboost_binary_testset(x_test, times):
    """Construct XGBoost test dataset using evaluation times for binary classification.
    ...
    Returns:
    - X_xgboost_test: DataFrame of features for XGBoost test set
    ...
    X_xgboost_test = pd.DataFrame(test_rows, columns=feature_cols)

    return X_xgboost_test, np.array(patient_ids)

def clean_column_names(df):
    """Clean column names to be compatible with XGBoost"""

    # Replace problematic characters and spaces
    clean_names = df.columns.str.replace('[', '_').str.replace(']', '_').str.replace('<', '_')
...
for file_name in dataset_files:
    try:
        ...
        x_val_ohe = clean_column_names(x_val_ohe) 
        ...
        # Train and validate using XGBoost model
        # Construct XGBoost training set
        X_xgb_train, y_xgb_train = construct_xgboost_binary_trainset(
        ...
        # Construct XGBoost validation set
        X_xgboost_val, val_patient_ids = construct_xgboost_binary_testset(
            x_val_imputed, eval_times
        )

        # Predict survival probabilities 
        probs = xgb_model.predict_proba(X_xgboost_val)
        surv_probs = probs[:, 0]  
        ...
        for i, t in enumerate(eval_times):
            mask = np.isclose(X_xgboost_val["eval_time"], t)
        ...
        # Clean column names 
        x_trainval_ohe = clean_column_names(x_trainval_ohe)
        x_test_ohe = clean_column_names(x_test_ohe)
        ...
        # Construct XGBoost training set
        X_xgboost_train, y_xgboost_train = construct_xgboost_binary_trainset(
        ...
        xgb_model.fit(X_xgboost_train, y_xgboost_train)

        # Construct XGBoost test set
        X_xgboost_test, test_patient_ids = construct_xgboost_binary_testset(
            x_test_filtered, eval_times
        )

        # Predict survival probabilities
        probs = xgb_model.predict_proba(X_xgboost_test)
        surv_probs = probs[:, 0]
        ...
        for i, t in enumerate(eval_times):
            mask = np.isclose(X_xgboost_test["eval_time"], t)
