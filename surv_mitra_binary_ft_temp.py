...
# CSV path
csv_path = "mitra_evaluation_ft_temp.csv"
...
for file_name in dataset_files:
    try:
        ...
        # Initialize and train Mitra model
        ...
        # Construct MITRA validation set
        X_mitra_val, val_patient_ids = construct_mitra_binary_testset(
            x_val_imputed, eval_times
        )

        # Convert val data to TabularDataset 
        val_data = TabularDataset(pd.DataFrame(X_mitra_val))
        ...
        # We'll use the raw probabilities as pseudo-logits for temperature calibration
        val_labels = []
        for i, t in enumerate(eval_times):
            mask = np.isclose(X_mitra_val["eval_time"], t)
        ...
        calibrated_probs = apply_temperature_scaling(probs, optimal_temperature)
        surv_probs = calibrated_probs[:, 0]
        ...
        # Store the optimal temperature for use in final test evaluation
        print(f"Validation C-index with temperature scaling: {val_c_index:.4f}")        
        ...
        # Use the temperature found during validation for final test predictions
