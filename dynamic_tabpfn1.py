...
def prepare_evaluation_data(df: pd.DataFrame, times):
    ...
    before that evaluation time. Creates a dataset with eval_time column.
    """
    eval_rows = []
...
def patient_risk_from_online_eval(online_out: pd.DataFrame):
    """
    Convert per-eval-time p_event into a single per-patient risk score:
      risk_i = 1 - prod_t (1 - p_event_{i,t})
    """
    risk_rows = []
    for pid, g in online_out.sort_values(["pid", "eval_time"]).groupby("pid", sort=False):
        s = np.prod(1.0 - g["p_event"].values)  # cumulative survival across eval times
        risk = float(1.0 - s)
